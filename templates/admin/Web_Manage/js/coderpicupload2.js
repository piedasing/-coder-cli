	...
	return this.each(function() {
			...
			parent.find('img').click(function(){
				if($(this).attr('data-filepath') && $(this).attr('data-picname')){
					if(_settings.jcrop){
						...
						openPicBox({href:$(this).attr('src'),initialWidth:'50px',initialHeight:'50px'});
					}

				}else{
					openPicBox({href:$(this).attr('src'),maxWidth: "95%",initialWidth:'50px',initialHeight:'50px'});
				}
			})
			initButton();
		...
		function showPics(pic,croptag){
			...
			if(pics!=null){
				...
				_pic.attr({'src':pic['filepath']+pic['filename']+'?time='+new Date().getTime()});
			}
		}
