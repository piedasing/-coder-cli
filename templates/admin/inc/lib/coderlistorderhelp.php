...
class coderlistorderhelp{
	...
	public static function dochangeOrder($method,$orderDesc,$table,$orderColumn,$order_id,$order_key,$prev_id,$where='',$add=5){
		if($method=='up' || $method=='down'){
			...
			self::changeSortableOrder($table,$order_id,$order_key,$prev_id,$orderColumn,$orderDesc,$where,$add);
		}
	}
	...
	public static function changeSortableOrder($table,$_thisid,$_thiskey,$_previd,$field,$orderDesc,$where,$add=5){
		global $db;
		if($_previd>0){//非第一筆
			$prevrow=$db->query_first("select $field from $table WHERE $_thiskey='".$_previd."' $where");
			$prevind=$prevrow[$field];//目標位置的前一筆ind
		}else{
			if($orderDesc=="desc"){
				$prevrow=$db->query_first("select $field from $table ".($where!=""?"where 1 $where":"")." ORDER BY $field DESC");
				$prevind=(int)$prevrow[$field]+$add;//目標位置的前一筆ind
			}else{
				$prevrow=$db->query_first("select $field from $table ".($where!=""?"where 1 $where":"")." ORDER BY $field ASC");
				$prevind=(int)$prevrow[$field]-$add;//目標位置的前一筆ind
			}
		}
		$thisrow=$db->query_first("select $field from $table WHERE $_thiskey='".$_thisid."' $where");
		$thisind=$thisrow[$field];//目標項目原本的ind

		if($orderDesc=="desc"){
			//1.如果目標往下移動($thisind>$prevind)， $thisind > db內ind && db內ind>=$prevind 增加排序值
			if($thisind>$prevind){
				$db->exec("update $table set $field=$field+$add where $field>=$prevind AND $field<$thisind $where");
				$db->exec("update $table set $field=$prevind where $_thiskey='$_thisid' $where");
			}else if($thisind<$prevind){
				$db->exec("update $table set $field=$field-$add where $field<$prevind AND $field>$thisind $where");
				$db->exec("update $table set $field=$prevind-$add where $_thiskey='$_thisid' $where");
			}
		}else{
