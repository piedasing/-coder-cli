...
class coderFormHelp {
    ...
    */
            ...
            for($i=1,$num=((!empty($item["repeat"]) && substr($key,-2,2)=='[]')?$item["repeat"]:1);$i<=$num;$i++){
                ...
                switch ($item["type"]) {
                    case "text": //文字欄位
                        ...
                        break;
                    case "datepicker":
                        $str .= self::drawForm_datepicker($key, $item, $attr,$i);
                        break;
                    case "colorpicker":
                    ...
                    case 'textarea': //textarea
                        $attr['class'].= ' form-control';
                        $attr['rows']=isset($item['rows']) ? $item['rows'] : 3;
                    ...
                    case "html2": //tiny editor
                            $attr['class'] .= ' form-control tiny-editor';
                            $str = self::drawForm_Textarea($key, $attr, $i);
                            break;
    ...
    public static function getSendDataValue($key, $item) {
        ...
        if ((isset($item['htmlencode']) && $item['htmlencode']==false) || $item['type'] == 'html' || $item['type'] == 'html2'){
            if ($item['type'] == 'html' || $item['type'] == 'html2') {
    ...
    public static function drawForm_Text($id, $attr = array(), $num = 0) {
    ...
    public static function drawForm_datepicker($id, $item, $attr,$num) {
        ...
        $placeholder = $items['placeholder'] ?? $placeholders[$viewMode];
        $str = '
            <div style="position: relative;">
                <label class="input-group">
                    ...
                    <input
                        ...
                        ' . self::getAttrStr($attr) . '
                        data-mask="9999-99-99"
                        data-date-viewmode="' . ($viewMode) . '"
                        data-date-format="' . ($formats[$viewMode]) . '"
                        value="' . $defval . '"
                        onkeydown="return;"
                    />
                </label>
    ...
    public static function drawForm_file($id, $attr = array(), $num = 0) {
    ...
    public static function drawForm_Hidden($id, $attr = array(),$num = 0) {

        return '<input type="hidden" id="' . ($num!==1?$id.$num:$id) . '" name="' . $id . '" ' . self::getAttrStr($attr) . '>';
    }
    public static function drawForm_Password($id, $attr = array(),$num = 0) {
        $ary_format = self::getTextFormat($attr); //取得text特殊樣式的範本
        $str = '<input type="password" id="' . ($num!==1?$id.$num:$id) . '" name="' . $id . '" ' . self::getAttrStr($attr) . '>';

        return $ary_format[0] . $str . $ary_format[1];
    }
    public static function drawForm_Checkbox($id, $attr = array(),$num = 0) {
    ...
    private static function drawForm_Select($id, $item, $attr,$num) {
        ...
        if (!isset($item['mode']) || $item['mode'] != 'no_default') {
    ...
    public static function drawForm_picgroup($id, $attr = array(),$num = 0) {
    ...
    if (jQuery().validate) {
        ...
        jQuery.validator.addMethod("taxid",
           function (value, element, param) {
                if(param=="yes"){
                    return CheckCompanyNo(value);
                }return true;
           }, $.validator.format("請輸入正確的統一編號"));
	}
	';
        if($items!=null){
    ...
    public function chkValid(&$err, $value, $item ,$data) {
        ...
        if($isrequired==true || trim($value) != ""){ // 必填或值不為空時才檢查格式,非必填且沒值時不用檢查
            ...
            if (!$isnum && isset($item['validate']['minlength']) && mb_strlen(trim($value), 'UTF-8') < intval($item['validate']['minlength'])) {
                $err[] = $item['name'] . '的長度不能小於' . $item['validate']['minlength'];
            }
            if (!$isnum && isset($item['validate']['maxlength']) && mb_strlen(trim($value), 'UTF-8') > intval($item['validate']['maxlength'])) {
    ...
    public static function moveCopyPic($file, $temp, $path, $resize='', $croptag='changepics_') {
        ...
        if (file_exists($temp . $file) && !file_exists($path . $file)) {
    ...
    public static function drawPicScript($method, $file_path, $pic, $varname) {
        if ($method == 'edit' && trim($pic) != '') {
            ...
            echo 'var ' . $varname . '=new Array();'.$varname.'["filepath"]="' . $file_path . '";'.$varname.'["filename"]="' . $pic . '";'.$varname.'["width"]="' . ($size[0] ?? '0') . '";'.$varname.'["height"]="' . ($size[1] ?? '0') . '";';
        } else {
    ...
    $phpfile = p後端php檔案
     */
    public static function selectBoxManipulation_ajax($selectbox,$val_input = '',$phpfile = null) {
