...
class imgUploder
{
    ...
    function __construct($Ufile)
    {
        ...
        $this->file_name = $this->hc($Ufile['name']);
        $this->file_sname = "";
    ...
    function chk_Style()
    {
        ...
        if ($this->fstyle == "zip") {
            if ($r[1] != "zip") {
                $this->user_msg = $this->user_msg . "此檔案不是zip壓縮檔<br>";
            }
        }
        ...
        if ($this->fstyle == "video") {
            if ($this->fileex != "ogg" && $this->fileex != "mpeg4" && $this->fileex != "webm" && $this->fileex != "mp4" && $this->fileex != "avi") {
    ...
    function createSmailImg($w, $h, $key, $target_path = '', $target_image = '', $smallnomake = false)
    { //建立小圖function
        ...
        if ($key == 0) {
            ...
            if ($key == 7) { // 依寬度等比縮小
                ...
                $tallw = $w2;
                $tallh = $h2;
            } else if ($key == 99) { //不裁，維持在寬高最大1920間
                if ($size[0] <= 1920 && $size[1] <= 1920) { //如果寬高都比1920小..則不縮圖
                    $w2 = $size[0];
                    $h2 = $size[1];
                    $w = $size[0];
                    $h = $size[1];
                } else if ($size[0] > 1920) { //如果寬>1920,依寬度等比縮小

                    $w2 = 1920;
                    $h2 = (1920 / $size[0]) * $size[1];

                } else { //如果高>1920,依高度等比縮小
                    $h2 = 1920;
                    $w2 = (1920 / $size[1]) * $size[0];
                }
                $tx = 0;
                $ty = 0;
                $sx = 0;
